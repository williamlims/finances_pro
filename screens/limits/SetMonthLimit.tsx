...
export function SetMonthLimit() {
        ...
        db.transaction((tx) => {
            tx.executeSql(
                ...
                if (results.rows.length > 0) {
                    const mensal = results.rows.item(0).mensal;
                    setLimiteAtual(mensal);
                }
                },
                (tx, error) => {
    ...
    const atualizarLimite = async () => {
        ...
        try {
            db.transaction((tx) => {
                tx.executeSql(
                    'UPDATE limites SET mensal = ? WHERE id=1',
                    [ limite ],
                    (tx, results) => {
    ...
    return (
        <>
            <Header title='Limite Mensal' />
            <ScrollView style={{ flex: 1, padding: 10}}>
                                                
                <View style={{ flexDirection: 'column', gap: 10, justifyContent: 'space-between'}}> 
                    
                    <Card containerStyle={{ marginHorizontal: 0 }}>
                        <Text style={{ textAlign: 'center', fontWeight: 'bold', color: '#0f3762', fontSize: 18 }}>LIMITE MENSAL</Text>
                        <Card.Divider />
                        <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'flex-start'}}>
                            <Text style={{ fontSize: 16 }}>
                                Nesta tela é possível ajustar o limite mensal. O ajuste se refere ao percentual (%) dos seus ganhos e pode ser definido de 1 a 100.
                            </Text>
                        </View>
                    </Card>
